...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_PlayerEditor : Editor
	{
		...
		protected string _feedbackListOriginalLabel;
		...
		protected virtual void DrawDurationAndDirection()
		{
			...
			if (TargetMmfPlayer.Direction == MMF_Player.Directions.BottomToTop)
			{
				...
				{
					TargetMmfPlayer.ChangeDirection();
				}
			}
			else
			{
		...
		protected virtual void DrawDebugControls()
		{
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				// reverse button
				if (GUILayout.Button(_changeDirectionText, EditorStyles.miniButtonMid))
				{
					(target as MMF_Player).ChangeDirection();
				}
			}
			EditorGUILayout.EndHorizontal();
		...
		protected virtual void DrawFeedbackHeader(int i)
		{
			...
			// handle original label in parenthesis
			if (_feedbackListFeedback.Label != _feedbackListFeedback.OriginalLabel)
			{
				if (_feedbackListFeedback.OriginalLabel == "")
				{
					_feedbackListFeedback.OriginalLabel = FeedbackPathAttribute.GetFeedbackDefaultName(_feedbackListFeedback.GetType());
				}
				if (_feedbackListFeedback.OriginalLabel != "")
				{
					_feedbackListLabel += " <color="+_originalLabelColor+">(" + _feedbackListFeedback.OriginalLabel + ")</color>";	
				}
			}
